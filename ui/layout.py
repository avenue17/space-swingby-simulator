...
def build_app():
    ...
    with gr.Blocks(title="3D Solar Swingby Simulator") as app:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("## 우주선 설정")
                angle = gr.Slider(0, 360, value=DEFAULT_LAUNCH_ANGLE, step=1, label="발사각")
                speed = gr.Slider(1, 45, value=DEFAULT_LAUNCH_SPEED, step=0.5, label="초기 속도")
                start_time = gr.Slider(0, 500, value=0, step=1, label="출발 시각")
                duration = gr.Slider(50, 700, value=DEFAULT_DURATION, step=10, label="시뮬레이션 시간")
                camera_mode = gr.Radio(["angled", "top", "intro"], value="angled", label="카메라 시점")
                ...
                mass_scale = gr.Slider(0.1, 5.0, value=1.0, step=0.1, label="질량 배율")
                radius_scale = gr.Slider(0.5, 3.0, value=1.0, step=0.1, label="반지름 배율")
                influence_scale = gr.Slider(0.5, 3.0, value=1.0, step=0.1, label="중력권 배율")
            ...
            with gr.Column(scale=3):
                solar_plot = gr.Plot(value=initial_fig, label="태양계 시뮬레이션")
                result_text = gr.Textbox(value=initial_summary, label="시뮬레이션 결과", lines=8)
                speed_plot = gr.Plot(value=initial_speed_fig, label="속도 그래프")
        ...
        for component in [angle, speed, start_time, duration, camera_mode]:
            component.change(
